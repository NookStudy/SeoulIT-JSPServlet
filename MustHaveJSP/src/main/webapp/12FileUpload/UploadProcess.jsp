...
//업로드할 파일의 최대용량제한(1mb로 지정함)..서버용량부족
...
try{
	/*
		앞에서 준비한 3개의 인수 request내장객체까지를 이용해서
	...
	*/
	...
	/*
		파일명에서 앞의 .(닷)을 찾아 인덱스를확인 한 후 확장자를 잘라낸다.
		확장자는 파일의 용도를 나타내는 부분이므로 중요한다.
	...
	*/
	...
	/*
		...
		윈도위의 경우 \(역슬래쉬), 리눅스의 경우/(슬러쉬)를 자동으로 붕한다.
		File객체를 각각 생성한 후 renameTo() 메서드르 통해서 변경한다.
	*/
	...
	/*
		...
		객체는 다르지만 폼값을 받기위해 메서드명은 동일해야 한다.
	*/
	...
	if(cateArray==null){
		...
		cateBuf.deleteCharAt(cateBuf.length()-2);
	}
	...
	/*
		파일업로드에 실패한 경우에는 request 영역에 에러메세지를 저장한 후 
		업로드폼으로 포워드한다.
	*/
	e.printStackTrace();
