...
/*
	...
	응답헤더에 날짜를 지정하는 경우 대한민국은 세계표준시 +09
	즉, 9시간이 느리므로 9시간을 더해주어야 정상적인 날짜가 표시된다.
	만약 9시이전의 시간으로 설정되면 어제날짜가 출력된다.
*/
...
/*
	addDateHeader(헤더명, long타입의 타임스탬프): 
		응답헤더에 날짜형식을 추가하는 경우 long타입의 타임스탬프로 변환 후 추가해야한다. 
*/
//날짜 형식의 응답헤더 추가
response.addDateHeader("myBirthday", add_date);
//정수 형식의 응답헤더 추가(동일한 헤더명으로 2개를 추가함)
response.addIntHeader("myNumber", add_int);
response.addIntHeader("myNumber", 1004); //추가
//문장열형식의 응답헤더 추가
response.addHeader("myName", add_str);
//기존의 응답헤더를 '안중근'으로 수정합니다.
...
<body>
	...
	/*
		첫번째 출력결과에서 myNumber라는 헤드명이 2번 출력되는데 이때
