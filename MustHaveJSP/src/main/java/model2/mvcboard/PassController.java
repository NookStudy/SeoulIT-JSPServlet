...
public class PassController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		/*
		 	/mvcboard/pass.do?mode=delete&idx=118  : 118번 게시물 삭제
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (confirmed) {
			/*
			 	...
			 	session영역은 페이지를 이동해도 데이터가 공유되므로 해당 게시물을 수정완료할때까지 유지하고
			 	수정이 완료되면 제거할 것이다.
			 	여기에서 저장한 패스워는 수정을 위한 update쿼리문의 where절에 사용할 것이다.
			 */
			...
			if (mode.equals("edit")) {
				...
				/*
				   여기서 get방식으로 이동하기 때문에 url만 알면 바로 진입가능. 
				   따라서 session영역에 저장된 pass로 EditController에서 한번더 검증해야 한다.
				   url패턴으로만 들어가면 session영역에 저장된 pass가 없으므로 수정처리가 불가능
				 */
