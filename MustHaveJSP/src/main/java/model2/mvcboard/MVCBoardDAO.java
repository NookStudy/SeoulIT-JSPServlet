...
public class MVCBoardDAO extends DBConnPool {
	// 생성자에서 DBCP(커넥션 풀)를 통해 오라클에 연결한다.
	public MVCBoardDAO() {
		super();
	}
...
//			System.out.println(rs.getInt(1));
	...
	/*
	 * 모델1 방식에서는 board테이블 및 BoardDTO클래스를 사용했지만 모델2 방식에서는 mvcboard 테이블 및 MVCBoardDTO
	 * 를 사용하므로 해당 코드만 수정하면 됩니다.
	 */
...
//		System.out.println("list query: "+query);
	...
	public int insertWrite(MVCBoardDTO dto) {
		...
		try {
			...
			String query = "insert into mvcboard(idx,name,title,content,ofile,sfile,pass) "
					+ "values(seq_mvc2board_num.nextval,?,?,?,?,?,?)";
			psmt = con.prepareStatement(query);
	...
	public MVCBoardDTO selectView(String idx) {
		...
		MVCBoardDTO dto = new MVCBoardDTO();
		// 쿼리문 작성후 인파라미터를 설정하고 실행한다.
		sql = "select * from mvcboard Where idx=?";
		try {
	...
	public void updateVisitCount(String idx) {
		sql = "update mvcboard set visitcount=visitcount+1 where idx=?";
		try {
			psmt = con.prepareStatement(sql);
			psmt.setString(1, idx);
			psmt.executeUpdate();
	...
	public void downCountPlus(String idx) {
		sql = "UPDATE MVCBOARD  set downcount=downcount+1 where idx=?";
		try {
			psmt = con.prepareStatement(sql);
			psmt.setString(1, idx);
			psmt.executeUpdate();
	...
	// 게시글 데이터를 받아 db에 저장된 내용을 갱신.(파일업로드 지원)
	public int updatePost(MVCBoardDTO dto) {
		...
		try {
			...
			// 레디 앤
			sql = "update mvcboard set title=?, name=?, content=?, ofile=?, sfile=?,  pass=? where idx=?";
			// 겟
			psmt = con.prepareStatement(sql);
			// 셋
